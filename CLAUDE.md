...
**Key difference from BonusRota:** BonusRota uses Supabase/Postgres; BonusUfku uses **Firebase Firestore** (Supabase org's free-tier project limit was hit, so this project moved to Firebase — see README.md).
...
- `api/telegram-webhook.js` — Vercel serverless function (no npm deps, uses global `fetch`) acting on the Telegram bot's webhook. On `/start`, sends a welcome message with an inline `web_app` button opening the site, and logs a `bot_start` event to Firestore via the REST API (no service account needed — Firestore rules are open, see below). Reads `TELEGRAM_BOT_TOKEN` and `TELEGRAM_WEBHOOK_SECRET` from Vercel environment variables (never hardcoded — repo is public). Validates the `X-Telegram-Bot-Api-Secret-Token` header against `TELEGRAM_WEBHOOK_SECRET`. The Telegram bot for BonusUfku had not been created yet as of this note — wire up `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET` and call Telegram's `setWebhook` once it exists.
- `firestore.rules` / `firebase.json` / `.firebaserc` — Firestore config; rules are wide open (`allow read, write: if true`) — "test mode", same risk tolerance as BonusRota's permissive Supabase RLS.
- `README.md` — Turkish deployment runbook.
...
- `sites` Firestore collection documents: `name, bonus, type, tag ('trend'|'popular'), link, logo, display_order, active`. Document IDs are Firestore's auto-generated string IDs (no numeric `id` like Supabase).
...
- Firestore rules changes must be applied manually (`firebase deploy --only firestore:rules` or via the Firebase console) and kept in sync with `firestore.rules` in this repo.
